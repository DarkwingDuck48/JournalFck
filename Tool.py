...
class Tool:
    def __init__(self, mapping=None, filename=None):
        ...
        with open("Mapping.json", "r", encoding="utf-8") as file:
            ...
            self.mappings = alldict["Mappings"]

        self.convertedJournals = open("ConvJourn2016_NovDec.txt", 'w', encoding="utf-8")
        self.log = open('logs.txt', 'w', encoding="utf-8")

        with open("1123", "r") as journal:
            for line in journal:
                if line.isspace():
                    self.convertedJournals.write(line)
                    ...
                    if line.strip().split(";")[0].isdigit():
                        for acc in self.mappings.keys():
                            if line.startswith(acc):
                                accountmap = self.mappings[acc]
                                mappedline = line.strip().split(";")
                                mstring = self.accountmapping(accountmap, mappedline) + "\n"
                                line = mstring
                                self.log.write(mstring)
        ...
        self.convertedJournals.close()
        self.log.close()
        print("Done!")

    def accountmapping(self, mapping, mString):
        line = mString
        if len(line) == 12:
